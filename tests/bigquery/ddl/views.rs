...
fn test_drop_materialized_view() {
    ...
    let result = executor.execute_sql("SELECT * FROM to_drop_mv");
    assert!(result.is_err());
}
...
fn test_drop_materialized_view_if_exists() {
    ...
    let result = executor.execute_sql("DROP MATERIALIZED VIEW IF EXISTS nonexistent_mv");
    assert!(result.is_ok());
}
...
fn test_view_with_qualified_name() {
    ...
    executor.execute_sql("CREATE SCHEMA view_schema").unwrap();
