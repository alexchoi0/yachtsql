...
fn test_between_integers() {
    ...
    let result = session.execute_sql("SELECT 5 BETWEEN 1 AND 10").unwrap();
...
fn test_between_lower_bound() {
    let mut session = create_session();
    let result = session.execute_sql("SELECT 1 BETWEEN 1 AND 10").unwrap();
    assert_table_eq!(result, [[true]]);
}
...
fn test_between_upper_bound() {
    let mut session = create_session();
    let result = session.execute_sql("SELECT 10 BETWEEN 1 AND 10").unwrap();
    assert_table_eq!(result, [[true]]);
}
...
fn test_between_outside_range() {
    let mut session = create_session();
    let result = session.execute_sql("SELECT 15 BETWEEN 1 AND 10").unwrap();
...
fn test_between_with_null() {
    let mut session = create_session();
    let result = session.execute_sql("SELECT NULL BETWEEN 1 AND 10").unwrap();
...
fn test_between_negative_numbers() {
    let mut session = create_session();
    let result = session.execute_sql("SELECT -5 BETWEEN -10 AND 0").unwrap();
    assert_table_eq!(result, [[true]]);
}
