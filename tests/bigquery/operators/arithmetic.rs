...
fn test_modulo() {
    let mut session = create_session();
    let result = session.execute_sql("SELECT 10 % 3").unwrap();
...
fn test_negative_result() {
    let mut session = create_session();
    let result = session.execute_sql("SELECT 3 - 10").unwrap();
...
fn test_operator_precedence_mul_add() {
    let mut session = create_session();
    let result = session.execute_sql("SELECT 2 + 3 * 4").unwrap();
...
fn test_complex_expression() {
    let mut session = create_session();
    let result = session.execute_sql("SELECT (10 + 5) / 3 * 2").unwrap();
