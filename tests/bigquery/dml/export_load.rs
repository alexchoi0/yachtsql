...
mod local_parquet {
    ...
    fn test_load_parquet_basic() {
        ...
        session.execute_sql(&load_sql).unwrap();
    ...
    fn test_export_parquet_basic() {
        ...
        session.execute_sql(&export_sql).unwrap();
        ...
        session2.execute_sql(&load_sql).unwrap();
    ...
    fn test_export_empty_result() {
        ...
        let result = session.execute_sql(&export_sql);
        assert!(result.is_ok());
    }
...
mod load_data_parsing {
    ...
    fn test_load_data_missing_uris_error() {
        ...
        let result = session.execute_sql("LOAD DATA INTO no_uris FROM FILES (format='CSV')");
        assert!(result.is_err());
    }
    ...
    fn test_load_data_missing_from_files_error() {
        ...
        let result = session.execute_sql("LOAD DATA INTO no_from");
        assert!(result.is_err());
    }
