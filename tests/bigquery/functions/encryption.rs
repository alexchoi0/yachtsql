...
fn test_keys_keyset_from_json() {
    ...
    let result =
        session.execute_sql("SELECT KEYS.KEYSET_FROM_JSON('{\"primaryKeyId\": 123, \"key\": []}')");
    assert!(result.is_ok() || result.is_err());
}
...
fn test_keys_keyset_to_json() {
    ...
    let result =
        session.execute_sql("SELECT KEYS.KEYSET_TO_JSON(KEYS.NEW_KEYSET('AEAD_AES_GCM_256'))");
    assert!(result.is_ok() || result.is_err());
}
...
fn test_aead_encrypt() {
    ...
    let result =
        session.execute_sql("SELECT AEAD.ENCRYPT(keyset, b'plaintext', b'additional_data')");
    assert!(result.is_ok() || result.is_err());
}
...
fn test_encrypt_column_data() {
    ...
    let result = session.execute_sql("SELECT id, name FROM users").unwrap();
...
fn test_keys_keyset_length() {
    ...
    let result =
        session.execute_sql("SELECT KEYS.KEYSET_LENGTH(KEYS.NEW_KEYSET('AEAD_AES_GCM_256'))");
    assert!(result.is_ok() || result.is_err());
}
...
fn test_error_function() {
    ...
    let result = session.execute_sql("SELECT ERROR('This is an error message')");
...
fn test_error_in_case() {
    ...
    let result =
        session.execute_sql("SELECT CASE WHEN 1 = 2 THEN 'ok' ELSE ERROR('Condition failed') END");
    assert!(result.is_err());
}
...
fn test_safe_divide() {
    ...
    let result = session.execute_sql("SELECT SAFE_DIVIDE(10, 0)").unwrap();
...
fn test_nullifzero() {
    ...
    let result = session.execute_sql("SELECT NULLIFZERO(0)").unwrap();
    assert_table_eq!(result, [[null]]);
}
...
fn test_zeroifnull() {
    ...
    let result = session.execute_sql("SELECT ZEROIFNULL(NULL)").unwrap();
