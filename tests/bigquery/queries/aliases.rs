...
fn test_column_alias() {
    ...
    let result = executor.execute_sql("SELECT 1 AS number").unwrap();
...
fn test_column_alias_without_as() {
    let mut executor = create_executor();
    let result = executor.execute_sql("SELECT 'hello' greeting").unwrap();
...
fn test_expression_alias() {
    let mut executor = create_executor();
    let result = executor.execute_sql("SELECT 2 + 3 AS sum_result").unwrap();
