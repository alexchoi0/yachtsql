...
fn test_for_system_time_variable() {
    ...
    let result = session.execute_sql("SELECT * FROM data FOR SYSTEM_TIME AS OF @snapshot_time");
    assert!(result.is_ok() || result.is_err());
}
...
fn test_assert_basic() {
    ...
    let result = session.execute_sql("ASSERT (1 = 1)");
    assert!(result.is_ok());
}
...
fn test_assert_with_message() {
    ...
    let result = session.execute_sql("ASSERT (1 = 1) AS 'Basic equality check'");
    assert!(result.is_ok());
}
...
fn test_assert_fails() {
    ...
    let result = session.execute_sql("ASSERT (1 = 2)");
...
fn test_assert_with_subquery() {
    ...
    let result = session.execute_sql("ASSERT ((SELECT COUNT(*) FROM check_data) = 3)");
    assert!(result.is_ok());
}
...
fn test_assert_not_null() {
    ...
    let result = session.execute_sql("ASSERT (COALESCE(1, 0) IS NOT NULL)");
    assert!(result.is_ok());
}
...
fn test_assert_comparison() {
    ...
    let result = session.execute_sql("ASSERT (5 > 3 AND 10 < 20) AS 'Range check'");
    assert!(result.is_ok());
}
