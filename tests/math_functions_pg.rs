...
fn setup_pg() -> QueryExecutor {
    ...
    executor
        ...
        .unwrap();
    executor
        .execute_sql("INSERT INTO products VALUES (4, 'Grape', 3.99, 50)")
        .unwrap();
    executor
        .execute_sql("INSERT INTO products VALUES (5, 'Melon', 5.00, 25)")
        .unwrap();
    ...
    executor
        ...
        .unwrap();
    executor
        .execute_sql("INSERT INTO points VALUES (4, 4.0, 8.0)")
        .unwrap();
    executor
        .execute_sql("INSERT INTO points VALUES (5, 5.0, 10.0)")
        .unwrap();
...
#[test]
fn test_pg_abs_sign_mod() {
    let mut executor = setup_pg();
    let result = executor
...
#[test]
fn test_pg_power_sqrt() {
    let mut executor = setup_pg();

    let result = executor
        .execute_sql("SELECT POWER(2, 10), SQRT(16), POWER(27, 1.0/3) FROM products LIMIT 1")
        .unwrap();
    assert_eq!(result.num_rows(), 1);
    ...
    assert!((get_numeric(col2.get(0).unwrap()) - 3.0).abs() < 0.01);
}

#[test]
fn test_pg_round_with_precision() {
    let mut executor = setup_pg();
...
#[test]
fn test_pg_round_column() {
    let mut executor = setup_pg();

    let result = executor
        .execute_sql("SELECT ROUND(price, 1) FROM products ORDER BY id")
        .unwrap();
    assert_eq!(result.num_rows(), 5);
}

#[test]
fn test_pg_trig_basic() {
    let mut executor = setup_pg();
    let result = executor
    ...
    assert!(col0.get(0).unwrap().as_f64().unwrap().abs() < 0.001);
    assert!((col1.get(0).unwrap().as_f64().unwrap() - 1.0).abs() < 0.001);
    assert!(col2.get(0).unwrap().as_f64().unwrap().abs() < 0.001);
}

#[test]
fn test_pg_inverse_trig() {
    let mut executor = setup_pg();
    let result = executor
    ...
    assert!((col0.get(0).unwrap().as_f64().unwrap() - 0.5236).abs() < 0.01);

    assert!((col1.get(0).unwrap().as_f64().unwrap() - 1.0472).abs() < 0.01);

    assert!((col2.get(0).unwrap().as_f64().unwrap() - 0.7854).abs() < 0.01);
}

#[test]
fn test_pg_atan2_degrees_radians() {
    let mut executor = setup_pg();
    let result = executor
        .execute_sql("SELECT ATAN2(1, 1), DEGREES(PI()), RADIANS(180) FROM products LIMIT 1")
        .unwrap();
    assert_eq!(result.num_rows(), 1);
    ...
    assert!((col0.get(0).unwrap().as_f64().unwrap() - 0.7854).abs() < 0.01);

    assert!((col1.get(0).unwrap().as_f64().unwrap() - 180.0).abs() < 0.01);

    assert!((col2.get(0).unwrap().as_f64().unwrap() - std::f64::consts::PI).abs() < 0.01);
}

#[test]
fn test_pg_log_ln_exp() {
    let mut executor = setup_pg();
...
#[test]
fn test_pg_log_with_base() {
    let mut executor = setup_pg();

    let result = executor
        .execute_sql("SELECT LOG(2, 8) FROM products LIMIT 1")
        .unwrap();
    assert_eq!(result.num_rows(), 1);

    let col0 = result.column(0).unwrap();
    assert!((col0.get(0).unwrap().as_f64().unwrap() - 3.0).abs() < 0.001);
}

#[test]
fn test_pg_stddev_variance() {
    let mut executor = setup_pg();
    let result = executor
    ...
    assert!(col0.get(0).unwrap().as_f64().unwrap() >= 0.0);
    assert!(col1.get(0).unwrap().as_f64().unwrap() >= 0.0);
}

#[test]
fn test_pg_corr_covar() {
    let mut executor = setup_pg();
    let result = executor
    ...
    assert!((col0.get(0).unwrap().as_f64().unwrap() - 1.0).abs() < 0.001);
...
fn test_pg_random() {
    ...
    let col0 = result.column(0).unwrap();
    let val = col0.get(0).unwrap().as_f64().unwrap();

    assert!(val >= 0.0 && val < 1.0);
}

#[test]
fn test_pg_random_multiple() {
    let mut executor = setup_pg();

    let result = executor
        .execute_sql("SELECT RANDOM() FROM products")
        .unwrap();
    assert_eq!(result.num_rows(), 5);

    let col0 = result.column(0).unwrap();
    for i in 0..result.num_rows() {
        let val = get_numeric(col0.get(i).unwrap());
        assert!(val >= 0.0 && val < 1.0);
    }
}

#[test]
fn test_pg_greatest_least() {
    let mut executor = setup_pg();
    let result = executor
    ...
    assert!((get_numeric(col0.get(1).unwrap()) - 25.0).abs() < 0.001);
    assert!((get_numeric(col1.get(1).unwrap()) - 5.0).abs() < 0.001);
}

#[test]
fn test_pg_constants() {
    let mut executor = setup_pg();

    let result = executor
        .execute_sql("SELECT PI(), EXP(1) FROM products LIMIT 1")
        .unwrap();
    assert_eq!(result.num_rows(), 1);
    ...
    assert!((get_numeric(col0.get(0).unwrap()) - std::f64::consts::PI).abs() < 0.0001);
    assert!((get_numeric(col1.get(0).unwrap()) - std::f64::consts::E).abs() < 0.0001);
}
...
#[test]
fn test_pg_expression_composition() {
    let mut executor = setup_pg();

    let result = executor
        .execute_sql("SELECT SQRT(POWER(x, 2) + POWER(y, 2)) as distance FROM points ORDER BY id")
        .unwrap();
    assert_eq!(result.num_rows(), 5);

    let col0 = result.column(0).unwrap();

    assert!((col0.get(0).unwrap().as_f64().unwrap() - 2.236).abs() < 0.01);
}
