...
fn test_alter_table_drop_constraint() {
    ...
    executor
        .execute_sql("ALTER TABLE users DROP CONSTRAINT users_email_key")
        .unwrap();
...
fn test_alter_table_set_default() {
    ...
    executor
        .execute_sql("ALTER TABLE users ALTER COLUMN status SET DEFAULT 'active'")
        .unwrap();
    ...
    let result = executor.execute_sql("SELECT * FROM users").unwrap();
    assert_table_eq!(result, [[1, "active"]]);
}
...
fn test_alter_table_drop_default() {
    ...
    executor
        .execute_sql("ALTER TABLE users ALTER COLUMN status DROP DEFAULT")
        .unwrap();
