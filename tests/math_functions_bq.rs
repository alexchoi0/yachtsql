...
#[test]
fn test_bq_abs_sign_mod() {
    let mut executor = setup_bq();
    let result = executor
...
#[test]
fn test_bq_atan2_degrees_radians() {
    let mut executor = setup_bq();
    let result = executor
    ...
    assert!((get_numeric(col0.get(0).unwrap()) - 0.7854).abs() < 0.01);
    assert!((get_numeric(col1.get(0).unwrap()) - 180.0).abs() < 0.01);
    assert!((get_numeric(col2.get(0).unwrap()) - std::f64::consts::PI).abs() < 0.01);
}

#[test]
fn test_bq_log_ln_exp() {
    let mut executor = setup_bq();
    let result = executor
...
fn test_bq_log_with_base() {
    ...
    let col0 = result.column(0).unwrap();
    assert!((get_numeric(col0.get(0).unwrap()) - 3.0).abs() < 0.001);
}

#[test]
fn test_bq_stddev_variance() {
    let mut executor = setup_bq();
    let result = executor
...
#[test]
fn test_bq_expression_composition() {
    let mut executor = setup_bq();
    let result = executor
    ...
    let col0 = result.column(0).unwrap();
    assert!((get_numeric(col0.get(0).unwrap()) - 2.236).abs() < 0.01);
}
