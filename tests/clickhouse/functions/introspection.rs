...
fn test_is_infinite() {
    let mut executor = create_executor();
    let result = executor.execute_sql("SELECT isInfinite(1.0)").unwrap();
    assert!(result.num_rows() == 1);
}
...
fn test_is_nan() {
    let mut executor = create_executor();
    let result = executor.execute_sql("SELECT isNaN(1.0)").unwrap();
    assert!(result.num_rows() == 1);
}
...
fn test_default_value_of_argument_type() {
    let mut executor = create_executor();
    let result = executor
        .execute_sql("SELECT defaultValueOfArgumentType(1)")
        .unwrap();
    assert!(result.num_rows() == 1);
}
...
fn test_is_decimal_overflow() {
    let mut executor = create_executor();
    let result = executor
        .execute_sql("SELECT isDecimalOverflow(1.5)")
        .unwrap();
    assert!(result.num_rows() == 1);
}
