...
fn test_is_infinite() {
    let mut executor = create_executor();
    let result = executor.execute_sql("SELECT isInfinite(inf)").unwrap();
    assert_table_eq!(result, [[true]]);
}
...
fn test_is_nan() {
    let mut executor = create_executor();
    let result = executor.execute_sql("SELECT isNaN(nan)").unwrap();
    assert_table_eq!(result, [[true]]);
}
...
fn test_run_callback_in_final() {
    let mut executor = create_executor();
    let result = executor
        .execute_sql("SELECT runningAccumulate(sum_state) FROM (SELECT sumState(1) AS sum_state)")
        .unwrap();
    assert!(result.num_rows() == 1); // TODO: use table![[expected_values]]
}
