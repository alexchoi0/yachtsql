...
pub struct QueryExecutor {
    ...
    temp_functions: HashMap<String, TempFunction>,
}
...
impl QueryExecutor {
    ...
    fn expand_temp_functions(&self, expr: &Expr) -> Expr {
        match expr {
            Expr::Function(func) => {
                let func_name = func.name.to_string().to_uppercase();
                if let Some(temp_func) = self.temp_functions.get(&func_name) {
                        ...
                        ast::FunctionArguments::List(list) => list
                            ...
                            .filter_map(|arg| match arg {
                                ast::FunctionArg::Unnamed(ast::FunctionArgExpr::Expr(e)) => {
                                    Some(self.expand_temp_functions(e))
                                }
                    ...
                    let substituted =
                        self.substitute_params_in_expr(&temp_func.body, &temp_func.params, &args);
                    return self.expand_temp_functions(&substituted);
                }
                let mut new_func = func.clone();
                new_func.args = match &func.args {
                    ast::FunctionArguments::None => ast::FunctionArguments::None,
                    ast::FunctionArguments::Subquery(q) => {
                        ast::FunctionArguments::Subquery(q.clone())
                    }
            ...
            Expr::BinaryOp { left, op, right } => Expr::BinaryOp {
                left: Box::new(self.expand_temp_functions(left)),
                op: op.clone(),
                right: Box::new(self.expand_temp_functions(right)),
            },
            Expr::UnaryOp { op, expr: inner } => Expr::UnaryOp {
                op: *op,
                expr: Box::new(self.expand_temp_functions(inner)),
            },
            Expr::Nested(inner) => Expr::Nested(Box::new(self.expand_temp_functions(inner))),
            Expr::Case {
                case_token,
                end_token,
                operand,
                ...
                operand: operand
                    .as_ref()
                    .map(|e| Box::new(self.expand_temp_functions(e))),
                conditions: conditions
                    .iter()
                    .map(|cw| ast::CaseWhen {
                        condition: self.expand_temp_functions(&cw.condition),
                        result: self.expand_temp_functions(&cw.result),
                    })
                    .collect(),
                else_result: else_result
                    .as_ref()
    ...
    fn substitute_params_in_expr(
        ...
        match expr {
                        ...
                        ast::FunctionArguments::List(ast::FunctionArgumentList {
                            ...
                            args: list
                                ...
                                .map(|arg| match arg {
                                    ast::FunctionArg::Unnamed(ast::FunctionArgExpr::Expr(e)) => {
                                        ast::FunctionArg::Unnamed(ast::FunctionArgExpr::Expr(
                                            self.substitute_params_in_expr(e, params, args),
                                        ))
                                    }
                                    ast::FunctionArg::Named {
                                        name,
            ...
            Expr::BinaryOp { left, op, right } => Expr::BinaryOp {
                left: Box::new(self.substitute_params_in_expr(left, params, args)),
                op: op.clone(),
                right: Box::new(self.substitute_params_in_expr(right, params, args)),
            },
            Expr::UnaryOp { op, expr: inner } => Expr::UnaryOp {
                op: *op,
                expr: Box::new(self.substitute_params_in_expr(inner, params, args)),
            },
            Expr::Nested(inner) => Expr::Nested(Box::new(
                self.substitute_params_in_expr(inner, params, args),
            )),
            Expr::Case {
                case_token,
                end_token,
                ...
                operand: operand
                    .as_ref()
                    .map(|e| Box::new(self.substitute_params_in_expr(e, params, args))),
                conditions: conditions
                    .iter()
                    .map(|cw| ast::CaseWhen {
                        condition: self.substitute_params_in_expr(&cw.condition, params, args),
                        result: self.substitute_params_in_expr(&cw.result, params, args),
                    })
                    .collect(),
                else_result: else_result
                    .as_ref()
    ...
    fn execute_select_with_ctes(
        ...
        let (schema, mut rows) = if do_projection {
            let (schema, mut projected_rows) = self.project_rows_with_ctes(
        ...
        for (idx, item) in select.projection.iter().enumerate() {
            match item {
                SelectItem::UnnamedExpr(expr) => {
                    let expanded_expr = self.expand_temp_functions(expr);
                    let resolved_expr =
                        self.resolve_scalar_subqueries_with_ctes(&expanded_expr, cte_tables)?;
                    let val =
    ...
    fn evaluate_select_with_from(&self, select: &Select) -> Result<(Schema, Vec<Record>)> {
        ...
        let mut filtered_rows: Vec<Record> = if let Some(selection) = &select.selection {
            ...
            if self.expr_contains_exists(&substituted_selection) {
                let mut result = Vec::new();
                for row in &input_rows {
                    if self.evaluate_exists_expr(
        ...
        if has_window_funcs || qualify_has_window {
            let expanded_projection =
                self.expand_named_windows_in_projection(&select.projection, &select.named_window);
            let expanded_qualify = select
                .qualify
                .as_ref()
                .map(|q| self.expand_named_windows_in_expr(q, &select.named_window));
            let window_results = self.compute_window_functions_with_qualify(
                &input_schema,
                &filtered_rows,
                &expanded_projection,
                expanded_qualify.as_ref(),
            )?;

            let rows_after_qualify = if let Some(qualify_expr) = &select.qualify {
                let expanded_qualify_expr =
                    self.expand_named_windows_in_expr(qualify_expr, &select.named_window);
                let qualifying_indices = self.apply_qualify_filter(
                    &input_schema,
                    &filtered_rows,
                    &expanded_qualify_expr,
                    &window_results,
                )?;
    ...
    fn evaluate_select_with_from_for_ordering_ctes(
        ...
        let mut filtered_rows: Vec<Record> = if let Some(selection) = &select.selection {
            ...
            if self.expr_contains_exists(&substituted_selection) {
                ...
                let resolved_selection =
                    self.resolve_scalar_subqueries_with_ctes(&substituted_selection, cte_tables)?;
                input_rows
                    .iter()
                    .filter(|row| {
                        evaluator
        ...
        if has_aggregates || has_group_by {
            let order_by_agg_exprs =
                self.get_order_by_aggregate_exprs_not_in_projection(order_by, &select.projection);

            if order_by_agg_exprs.is_empty() {
            ...
            let mut extended_projection = select.projection.clone();
            for (idx, expr) in order_by_agg_exprs.iter().enumerate() {
                let alias_name = format!("__order_agg_{}", idx);
                extended_projection.push(SelectItem::ExprWithAlias {
                    expr: expr.clone(),
                    alias: ast::Ident::new(alias_name),
                });
            }

            let extended_select = Select {
                projection: extended_projection,
                ..select.clone()
            };
        ...
        if has_window_funcs || qualify_has_window {
            ...
            let (final_rows, final_window_results) = if let Some(qualify_expr) = &select.qualify {
                let expanded_qualify_expr =
                    self.expand_named_windows_in_expr(qualify_expr, &select.named_window);
                let qualifying_indices = self.apply_qualify_filter(
                    &input_schema,
                ...
                (filtered, new_window_results)
            } else {
                (filtered_rows.clone(), window_results)
            };

            let needs_deferred = order_by.as_ref().is_some_and(|ob| {
                self.order_by_references_non_projected_columns(
                    &input_schema,
                    &select.projection,
                    ob,
                )
            ...
            if needs_deferred {
                let (extended_schema, extended_rows) = self.create_extended_rows_with_windows(
                    &input_schema,
                    &final_rows,
                    &expanded_projection,
                    &final_window_results,
                )?;
                return Ok((extended_schema, extended_rows, true));
            }
        ...
        if needs_deferred_projection {
            ...
            let (output_schema, mut output_rows) = self.project_rows_with_ctes(
                &input_schema,
                &filtered_rows,
                &select.projection,
                cte_tables,
            ...
            match &order_expr.expr {
                Expr::Identifier(ident) => {
                    ...
                    if !projected_columns.contains(&col_name) {
                        if input_schema.fields().iter().any(|f| {
                            let f_upper = f.name.to_uppercase();
                            f_upper == col_name || f_upper.ends_with(&format!(".{}", col_name))
                        }) {
                            return true;
                        }
                    }
                ...
                Expr::CompoundIdentifier(parts) => {
                    if let Some(first) = parts.first() {
                        ...
                        let is_struct_field = input_schema.fields().iter().any(|f| {
                            f.name.to_uppercase() == first_name
                                || f.name.to_uppercase().ends_with(&format!(".{}", first_name))
                        });
                        if is_struct_field && !projected_columns.contains(&first_name) {
                            return true;
                        }
                    }
    ...
    fn get_order_by_aggregate_exprs_not_in_projection(
        &self,
        order_by: &Option<OrderBy>,
        projection: &[SelectItem],
    ) -> Vec<Expr> {
        ...
        let exprs = match &order_by.kind {
            OrderByKind::Expressions(exprs) => exprs,
            OrderByKind::All(_) => return vec![],
        };

        let projected_expr_keys: std::collections::HashSet<String> = projection
            .iter()
            .filter_map(|item| match item {
                SelectItem::UnnamedExpr(expr) => Some(self.normalize_expr_key(expr)),
                SelectItem::ExprWithAlias { expr, .. } => Some(self.normalize_expr_key(expr)),
                _ => None,
            })
            .collect();
        ...
        let mut result = Vec::new();
        for order_expr in exprs {
            if self.expr_contains_aggregate(&order_expr.expr) {
                let key = self.normalize_expr_key(&order_expr.expr);
                if !projected_expr_keys.contains(&key) {
                    result.push(order_expr.expr.clone());
                }
            }
        }
        result
        ...
        match expr {
            Expr::Function(func) => {
                ...
                if let ast::FunctionArguments::List(args) = &func.args {
                    for arg in &args.args {
                        if let ast::FunctionArg::Unnamed(ast::FunctionArgExpr::Expr(e)) = arg {
                            if self.expr_contains_aggregate(e) {
                                return true;
                            }
                        }
                    }
            ...
            Expr::Nested(inner) => self.expr_contains_aggregate(inner),
                        ...
                        Schema::from_fields(
                            table_data
                                ...
                                .map(|f| {
                                    ...
                                    .with_source_table(prefix.clone())
                                })
                                .collect(),
                        )
                    } else {
    ...
    fn execute_aggregate_query_with_ctes(
        ...
        let mut pre_computed_types: Vec<(String, Option<DataType>)> = Vec::new();
        for (idx, item) in select.projection.iter().enumerate() {
            match item {
                SelectItem::UnnamedExpr(expr) => {
                    let name = self.expr_to_alias(expr, idx);
                    let inferred = self.infer_expr_type(expr, input_schema);
                    pre_computed_types.push((name, inferred));
                }
                SelectItem::ExprWithAlias { expr, alias } => {
                    let inferred = self.infer_expr_type(expr, input_schema);
                    pre_computed_types.push((alias.value.clone(), inferred));
                }
                SelectItem::Wildcard(_) => {
                    for (i, ge) in group_exprs.iter().enumerate() {
                        let inferred = self.infer_expr_type(ge, input_schema);
                        pre_computed_types.push((format!("_col{}", i), inferred));
                    }
                }
                _ => {}
            }
            ...
            if output_fields.is_none() {
                output_fields = Some(
                    row_values
                        ...
                        .enumerate()
                        .map(|(i, val)| {
                            let (name, inferred_type) = &pre_computed_types[i];
                            let data_type = match inferred_type {
                                Some(dt) if *dt != DataType::Unknown => dt.clone(),
                                _ => val.data_type(),
                            };
        ...
        if has_window_funcs && !window_col_indices.is_empty() {
            ...
            let expanded_substituted_projection = self
                .expand_named_windows_in_projection(&substituted_projection, &select.named_window);
            let window_results = self.compute_window_functions(
                &schema,
                &result_rows,
                &expanded_substituted_projection,
            )?;
            ...
            for (row_idx, row) in result_rows.iter_mut().enumerate() {
                ...
                for &col_idx in &window_col_indices {
                    let item = &expanded_substituted_projection[col_idx];
                    let expr = match item {
        ...
        if let Some(qualify_expr) = &select.qualify {
            ...
            let substituted_qualify =
                self.substitute_aggregates_in_qualify(&expanded_qualify_expr, &select.projection);
    ...
    fn normalize_expr_key_from_name(&self, name: &str) -> String {
        name.to_uppercase()
    }
    ...
    fn evaluate_aggregate_expr_with_grouping_ctes(
        ...
        match expr {
            ...
            Expr::Value(v) => self.sql_value_to_value(&v.value),
            Expr::Subquery(query) => {
                let outer_row = if !group_rows.is_empty() {
                    (*group_rows[0]).clone()
                } else {
                    let mut group_fields = Vec::new();
                    for (idx, expr) in group_exprs.iter().enumerate() {
                        let name = self.expr_to_alias(expr, idx);
                        let val = group_key.get(idx).cloned().unwrap_or(Value::null());
                        group_fields.push(Field::nullable(name, val.data_type()));
                    }
                    Record::from_values(group_key.to_vec())
                };
                let substituted_query =
                    self.substitute_outer_refs_in_query(query, input_schema, &outer_row);
                let mut merged_ctes = cte_tables.clone();
                let inner_ctes = self.materialize_ctes(&substituted_query.with)?;
                merged_ctes.extend(inner_ctes);
                let result = self.execute_query_with_ctes(&substituted_query, &merged_ctes)?;
    ...
    fn compute_aggregate(
        ...
        match name {
            ...
            "LOGICAL_OR" | "BOOL_OR" => {
                let expr = arg_expr.ok_or_else(|| {
                    Error::InvalidQuery("LOGICAL_OR requires an argument".to_string())
                })?;
            ...
            "LOGICAL_AND" | "BOOL_AND" | "EVERY" => {
                let expr = arg_expr.ok_or_else(|| {
                    Error::InvalidQuery("LOGICAL_AND requires an argument".to_string())
                })?;
                let mut result = true;
    ...
    fn project_rows(
        ...
        self.project_rows_with_ctes(input_schema, rows, projection, &HashMap::new())
    }

    fn project_rows_with_ctes(
        &self,
        input_schema: &Schema,
        rows: &[Record],
        projection: &[SelectItem],
        ...
        if let Some(result) = self.try_project_from_precomputed(input_schema, rows, projection) {
            return result;
        }

        if self.has_array_join(projection) {
            let resolved_projection =
                self.resolve_projection_subqueries_with_ctes(projection, cte_tables)?;
            ...
            match item {
                SelectItem::Wildcard(opts) => {
                    ...
                    for field in input_schema.fields() {
                        if !Self::field_in_except(&field.name, &except_cols) {
                ...
                SelectItem::QualifiedWildcard(name, opts) => {
                    let table_name = name.to_string().trim_end_matches(".*").to_string();
                    let except_cols = Self::get_except_columns(opts);
                    let replace_map = Self::get_replace_map(opts);
                    for field in input_schema.fields() {
                        let matches_source = field
                            .source_table
                            .as_ref()
                            .is_some_and(|t| t.eq_ignore_ascii_case(&table_name));
                        let matches_prefix = field
                            .name
                            .to_uppercase()
                            .starts_with(&format!("{}.", table_name.to_uppercase()));
                        let matches_table = matches_source || matches_prefix;
                        if matches_table && !Self::field_in_except(&field.name, &except_cols) {
                            if let Some(replace_expr) = replace_map.get(&field.name.to_lowercase())
                            {
                                let val = evaluator
                                    .evaluate(replace_expr, &sample_record)
                ...
                SelectItem::UnnamedExpr(expr) => {
                    let resolved_expr = self.resolve_scalar_subqueries_with_row_context(
                        expr,
                        cte_tables,
                        input_schema,
                        &sample_record,
                    ...
                    let val = evaluator
                        ...
                        .unwrap_or(Value::null());
                    let name = self.expr_to_alias(expr, idx);
                    let data_type = if let Some(inferred) = self.infer_expr_type(expr, input_schema)
                    {
    ...
    fn try_project_from_precomputed(
        &self,
        input_schema: &Schema,
        rows: &[Record],
        projection: &[SelectItem],
    ) -> Option<Result<(Schema, Vec<Record>)>> {
        let mut proj_col_names: Vec<String> = Vec::new();
        ...
        for (idx, item) in projection.iter().enumerate() {
            match item {
                SelectItem::Wildcard(_) => {
                    for field in input_schema.fields() {
                        if !field.name.starts_with("__proj_") {
                            proj_col_names.push(format!("__proj_{}", field.name));
                            output_col_names.push(field.name.clone());
                        }
                    }
                }
                SelectItem::QualifiedWildcard(name, _) => {
                    ...
                    for field in input_schema.fields() {
                        ...
                        let matches_prefix = field
                            ...
                            .starts_with(&format!("{}.", table_name.to_uppercase()));
                        let matches_table = matches_source || matches_prefix;
                        if matches_table && !field.name.starts_with("__proj_") {
                            proj_col_names.push(format!("__proj_{}", field.name));
                            output_col_names.push(field.name.clone());
                        }
                    }
                ...
                SelectItem::UnnamedExpr(expr) => {
                    let name = self.expr_to_alias(expr, idx);
                    proj_col_names.push(format!("__proj_{}", name));
                    output_col_names.push(name);
                }
                SelectItem::ExprWithAlias { alias, .. } => {
                    proj_col_names.push(format!("__proj_{}", alias.value));
                    output_col_names.push(alias.value.clone());
                }
            }
        }

        let mut proj_col_indices: Vec<usize> = Vec::new();
        for proj_name in &proj_col_names {
            let idx = input_schema
                .fields()
                .iter()
                .position(|f| &f.name == proj_name)?;
            proj_col_indices.push(idx);
        }

        let output_fields: Vec<Field> = proj_col_indices
            .iter()
            .zip(output_col_names.iter())
            .map(|(&idx, name)| {
                let field = &input_schema.fields()[idx];
                Field::nullable(name.clone(), field.data_type.clone())
            })
            .collect();
        let output_schema = Schema::from_fields(output_fields);

        let output_rows: Vec<Record> = rows
            .iter()
            .map(|row| {
                let values: Vec<Value> = proj_col_indices
                    .iter()
        ...
        Some(Ok((output_schema, output_rows)))
    }

    fn resolve_projection_subqueries(&self, projection: &[SelectItem]) -> Result<Vec<SelectItem>> {
        self.resolve_projection_subqueries_with_ctes(projection, &HashMap::new())
    }

    fn resolve_projection_subqueries_with_ctes(
        &self,
        projection: &[SelectItem],
        cte_tables: &HashMap<String, Table>,
        ...
        projection
            ...
            .map(|item| match item {
                SelectItem::UnnamedExpr(expr) => {
                    let resolved = self.resolve_scalar_subqueries_with_ctes(expr, cte_tables)?;
    ...
    fn sort_rows(&self, schema: &Schema, rows: &mut Vec<Record>, order_by: &OrderBy) -> Result<()> {
        ...
        let mut expr_col_indices: Vec<Option<usize>> = Vec::new();
        for oe in exprs {
            let key = self.normalize_expr_key(&oe.expr);
            let col_idx = schema
                .fields()
                .iter()
                .position(|f| {
                    let f_upper = f.name.to_uppercase();
                    f_upper == key || f_upper.ends_with(&format!(".{}", key))
                })
                .or_else(|| {
                    if self.expr_contains_aggregate(&oe.expr) {
                        let col_name = format!("__order_agg_{}", agg_counter);
                        agg_counter += 1;
            ...
            expr_col_indices.push(col_idx);
        }

        rows.sort_by(|a, b| {
            for (idx, order_expr) in exprs.iter().enumerate() {
                let (a_val, b_val) = if let Some(col_idx) = expr_col_indices[idx] {
    ...
    fn execute_create_function(&mut self, create: &ast::CreateFunction) -> Result<Table> {
        ...
        if create.temporary && (language == "SQL" || language.is_empty()) {
            ...
            let params: Vec<(String, DataType)> = create
                .args
                .as_ref()
                .map(|args| {
                    args.iter()
                        .filter_map(|arg| {
                            let param_name = arg.name.as_ref()?.value.to_uppercase();
                            let data_type = self.sql_type_to_data_type(&arg.data_type).ok()?;
                            Some((param_name, data_type))
                        })
                        .collect()
                })
                .unwrap_or_default();
            let temp_func = TempFunction { params, body: expr };
            self.temp_functions.insert(name, temp_func);
            return Ok(Table::empty(Schema::new()));
        }
        ...
        match expr {
            ...
            Expr::Function(func) => {
                ...
                if let ast::FunctionArguments::Subquery(q) = &func.args {
                    ...
                    let rows = result.to_records()?;
                    if func_name == "ARRAY" {
                        let is_as_struct = if let ast::SetExpr::Select(select) = q.body.as_ref() {
                            matches!(select.value_table_mode, Some(ast::ValueTableMode::AsStruct))
                        } else {
                            false
                        };
                        if is_as_struct {
                        ...
                        let new_list_args: Vec<ast::FunctionArg> = list
                            ...
                            .map(|arg| match arg {
                                ast::FunctionArg::Unnamed(arg_expr) => match arg_expr {
                                    ast::FunctionArgExpr::Expr(e) => {
                                        let resolved = self
                                            .resolve_scalar_subqueries_with_ctes(e, cte_tables)?;
            ...
            Expr::Case {
                ...
                let resolved_operand = operand
                    .as_ref()
                    .map(|o| self.resolve_scalar_subqueries_with_ctes(o, cte_tables))
                    .transpose()?
                    .map(Box::new);
                let resolved_conditions: Vec<ast::CaseWhen> = conditions
                    ...
                    .map(|cw| {
                        let resolved_cond =
                            self.resolve_scalar_subqueries_with_ctes(&cw.condition, cte_tables)?;
                        let resolved_result =
                            self.resolve_scalar_subqueries_with_ctes(&cw.result, cte_tables)?;
                        Ok(ast::CaseWhen {
                ...
                let resolved_else = else_result
                    .as_ref()
                    .map(|e| self.resolve_scalar_subqueries_with_ctes(e, cte_tables))
                    .transpose()?
                    .map(Box::new);
                Ok(Expr::Case {
                    case_token: case_token.clone(),
    ...
    fn resolve_scalar_subqueries_with_row_context(
        &self,
        expr: &Expr,
        cte_tables: &HashMap<String, Table>,
        outer_schema: &Schema,
        ...
        match expr {
            Expr::Subquery(query) => {
                let substituted_query =
                    self.substitute_outer_refs_in_query(query, outer_schema, outer_row);
                let mut merged_ctes = cte_tables.clone();
                let inner_ctes = self.materialize_ctes(&substituted_query.with)?;
                merged_ctes.extend(inner_ctes);
                let result = self.execute_query_with_ctes(&substituted_query, &merged_ctes)?;
                let rows = result.to_records()?;
                if rows.is_empty() {
                    return Ok(self.value_to_expr(&Value::null()));
                }
                if rows.len() != 1 || result.schema().field_count() != 1 {
                    return Err(Error::InvalidQuery(
                        "Scalar subquery must return exactly one row and one column".to_string(),
            ...
            Expr::Function(func) => {
                ...
                if let ast::FunctionArguments::Subquery(q) = &func.args {
                    let substituted_query =
                        self.substitute_outer_refs_in_query(q, outer_schema, outer_row);
                    let mut merged_ctes = cte_tables.clone();
                    let inner_ctes = self.materialize_ctes(&substituted_query.with)?;
                    merged_ctes.extend(inner_ctes);
                    let result = self.execute_query_with_ctes(&substituted_query, &merged_ctes)?;
                    let rows = result.to_records()?;
                    if func_name == "ARRAY" {
                        let is_as_struct = if let ast::SetExpr::Select(select) =
                            substituted_query.body.as_ref()
                        {
                            matches!(select.value_table_mode, Some(ast::ValueTableMode::AsStruct))
                        } else {
                            false
                        ...
                        let new_list_args: Vec<ast::FunctionArg> = list
                            ...
                            .map(|arg| match arg {
                                ast::FunctionArg::Unnamed(arg_expr) => match arg_expr {
                                    ast::FunctionArgExpr::Expr(e) => {
                                        let resolved = self
                                            .resolve_scalar_subqueries_with_row_context(
                                                e,
                                                cte_tables,
                                                outer_schema,
                                                outer_row,
            ...
            Expr::BinaryOp { left, op, right } => {
                let resolved_left = self.resolve_scalar_subqueries_with_row_context(
                    left,
                    cte_tables,
                    outer_schema,
                    outer_row,
                )?;
                let resolved_right = self.resolve_scalar_subqueries_with_row_context(
                    right,
                    cte_tables,
                    outer_schema,
                    outer_row,
            ...
            Expr::UnaryOp { op, expr: inner } => {
                let resolved = self.resolve_scalar_subqueries_with_row_context(
                    inner,
                    cte_tables,
                    outer_schema,
                    outer_row,
            ...
            Expr::Exists { subquery, negated } => {
                let substituted_query =
                    self.substitute_outer_refs_in_query(subquery, outer_schema, outer_row);
                let mut merged_ctes = cte_tables.clone();
                let inner_ctes = self.materialize_ctes(&substituted_query.with)?;
                merged_ctes.extend(inner_ctes);
                let result = self.execute_query_with_ctes(&substituted_query, &merged_ctes)?;
                ...
                Ok(self.value_to_expr(&Value::bool_val(result_bool)))
            }
            _ => Ok(expr.clone()),
        }
    }
    ...
    fn value_to_expr(&self, value: &Value) -> Expr {
        match value {
            ...
            Value::Struct(fields) => {
                let struct_values: Vec<Expr> = fields
                    .iter()
                    .map(|(name, val)| {
                        let inner_expr = self.value_to_expr(val);
                        Expr::Named {
                            expr: Box::new(inner_expr),
                            name: ast::Ident::new(name.clone()),
                        }
                    })
                    .collect();
                Expr::Struct {
    ...
    fn expr_contains_exists(&self, expr: &Expr) -> bool {
        match expr {
            Expr::Exists { .. } => true,
            Expr::BinaryOp { left, right, .. } => {
                self.expr_contains_exists(left) || self.expr_contains_exists(right)
            }
            Expr::UnaryOp { expr, .. } => self.expr_contains_exists(expr),
            Expr::Nested(inner) => self.expr_contains_exists(inner),
            _ => false,
        }
    }

    fn evaluate_exists_expr(
        &self,
        expr: &Expr,
        outer_schema: &Schema,
        outer_row: &Record,
        ...
        match expr {
            ...
            Expr::BinaryOp { left, op, right } => {
                let left_has_exists = self.expr_contains_exists(left);
                let right_has_exists = self.expr_contains_exists(right);

                match op {
                    ast::BinaryOperator::And => {
                        let left_result = if left_has_exists {
                            self.evaluate_exists_expr(left, outer_schema, outer_row, cte_tables)?
                        } else {
                            let evaluator = Evaluator::with_user_functions(
                                outer_schema,
                                self.catalog.get_functions(),
                            );
                            evaluator.evaluate_to_bool(left, outer_row).unwrap_or(false)
                        };
                        if !left_result {
                            return Ok(false);
                        }
                        if right_has_exists {
                            self.evaluate_exists_expr(right, outer_schema, outer_row, cte_tables)
                        } else {
                            let evaluator = Evaluator::with_user_functions(
                                outer_schema,
                                self.catalog.get_functions(),
                            ...
                            Ok(evaluator
                                .evaluate_to_bool(right, outer_row)
                                .unwrap_or(false))
                        }
                    }
                    ast::BinaryOperator::Or => {
                    ...
                    _ => {
                        ...
                        Ok(evaluator.evaluate_to_bool(expr, outer_row).unwrap_or(false))
                    }
                }
            }
            Expr::UnaryOp {
                ...
                let result =
                    self.evaluate_exists_expr(inner, outer_schema, outer_row, cte_tables)?;
                Ok(!result)
            }
            Expr::Nested(inner) => {
                self.evaluate_exists_expr(inner, outer_schema, outer_row, cte_tables)
            ...
            _ => {
                let evaluator =
                    Evaluator::with_user_functions(outer_schema, self.catalog.get_functions());
                Ok(evaluator.evaluate_to_bool(expr, outer_row).unwrap_or(false))
            }
        }
    }
    ...
    fn substitute_outer_refs_in_query(
        ...
        new_query.body =
            Box::new(self.substitute_outer_refs_in_set_expr(&query.body, outer_schema, outer_row));
        new_query
    }
        ...
        match set_expr {
            SetExpr::Select(select) => {
                ...
                if let Some(selection) = &select.selection {
                    new_select.selection = Some(self.substitute_outer_refs_in_expr(
                        selection,
                        outer_schema,
                        outer_row,
                    ));
                }
                new_select.from = select
                    .from
                    .iter()
                    .map(|twj| {
                        let new_relation = match &twj.relation {
                            ast::TableFactor::UNNEST {
                                ...
                                let new_exprs: Vec<Expr> = array_exprs
                                    .iter()
                                    .map(|e| {
                                        self.substitute_outer_refs_in_expr(
                                            e,
                                            outer_schema,
                                            outer_row,
                                        )
                        ...
                        ast::TableWithJoins {
                            ...
                            joins: twj.joins.clone(),
                        }
                    })
                    .collect();
                SetExpr::Select(Box::new(new_select))
            }
            _ => set_expr.clone(),
        }
    }
    ...
    fn substitute_outer_refs_in_expr(
        ...
        match expr {
            Expr::CompoundIdentifier(parts) => {
                if parts.len() == 2 {
                    let table_alias = parts[0].value.to_uppercase();
                    let column_name = parts[1].value.to_uppercase();
                    for (i, field) in outer_schema.fields().iter().enumerate() {
                        let field_upper = field.name.to_uppercase();
                        if field_upper == column_name
                            || field_upper == format!("{}.{}", table_alias, column_name)
                            || field
                                .source_table
                                .as_ref()
                                .is_some_and(|t| t.to_uppercase() == table_alias)
                                && field_upper.ends_with(&column_name)
                        {
                            return self.value_to_expr(&outer_row.values()[i]);
                        }
                    }
                }
                expr.clone()
            ...
            Expr::Identifier(ident) => {
                let col_name = ident.value.to_uppercase();
                for (i, field) in outer_schema.fields().iter().enumerate() {
                    if field.name.to_uppercase() == col_name {
                        return self.value_to_expr(&outer_row.values()[i]);
                    }
                }
                expr.clone()
        ...
        match expr {
            ...
            Expr::Function(func) => {
                ...
                match func_name.as_str() {
                        ...
                        if let ast::FunctionArguments::List(list) = &func.args {
                            for arg in &list.args {
                                if let ast::FunctionArg::Unnamed(ast::FunctionArgExpr::Expr(e)) =
                                    arg
                                {
                                    if let Some(dt) = self.infer_expr_type(e, schema) {
                                        if dt != DataType::Unknown {
                                            return Some(dt);
                                        }
                                    }
                    ...
                    | "LOGICAL_OR" | "LOGICAL_AND" | "BOOL_OR" | "BOOL_AND" | "EVERY" => {
                        Some(DataType::Bool)
                    }
                    _ => None,
                }
            ...
            Expr::BinaryOp { left, op, right } => match op {
                ...
                | ast::BinaryOperator::Modulo => {
                    ...
                    match (left_type, right_type) {
                        ...
                        (Some(DataType::Int64), Some(DataType::Int64)) => {
                            if matches!(op, ast::BinaryOperator::Divide) {
                                Some(DataType::Float64)
                            } else {
                                Some(DataType::Int64)
                            }
                ...
                | ast::BinaryOperator::GtEq
                | ast::BinaryOperator::And
                | ast::BinaryOperator::Or => Some(DataType::Bool),
                _ => None,
            },
            Expr::UnaryOp { op, expr } => match op {
                ...
                ast::UnaryOperator::Not => Some(DataType::Bool),
                _ => None,
            },
            Expr::Nested(inner) => self.infer_expr_type(inner, schema),
            Expr::Value(val) => match &val.value {
                ...
                ast::Value::Boolean(_) => Some(DataType::Bool),
                ast::Value::Null => None,
                _ => None,
            },
            Expr::Case {
                ...
                for cw in conditions {
                    if let Some(dt) = self.infer_expr_type(&cw.result, schema) {
                        if dt != DataType::Unknown {
                            return Some(dt);
                        }
                    }
                ...
                if let Some(else_expr) = else_result {
                    return self.infer_expr_type(else_expr, schema);
                }
                None
            }
            Expr::Cast { data_type, .. } => self.sql_type_to_data_type(data_type).ok(),
            _ => None,
        }
    }
    ...
    fn expr_has_window_function(&self, expr: &Expr) -> bool {
        match expr {
                ...
                if let ast::FunctionArguments::List(arg_list) = &func.args {
                    arg_list.args.iter().any(|arg| match arg {
                        ast::FunctionArg::Named { arg, .. } | ast::FunctionArg::Unnamed(arg) => {
                            match arg {
                                ast::FunctionArgExpr::Expr(e) => self.expr_has_window_function(e),
                                ast::FunctionArgExpr::Wildcard
                                | ast::FunctionArgExpr::QualifiedWildcard(_) => false,
                            }
                        }
                        ast::FunctionArg::ExprNamed { arg, .. } => match arg {
                            ast::FunctionArgExpr::Expr(e) => self.expr_has_window_function(e),
            ...
            Expr::Cast { expr, .. } => self.expr_has_window_function(expr),
            _ => false,
        }
    }

    fn resolve_window_type(
        &self,
        over: &ast::WindowType,
        named_windows: &[ast::NamedWindowDefinition],
    ) -> Option<ast::WindowSpec> {
        match over {
            ast::WindowType::WindowSpec(spec) => Some(spec.clone()),
            ast::WindowType::NamedWindow(name) => {
                let name_upper = name.value.to_uppercase();
                for def in named_windows {
                    if def.0.value.to_uppercase() == name_upper {
                        match &def.1 {
                            ast::NamedWindowExpr::WindowSpec(spec) => return Some(spec.clone()),
                            ast::NamedWindowExpr::NamedWindow(ref_name) => {
                                return self.resolve_window_type(
    ...
    fn expand_named_windows_in_expr(
        ...
        match expr {
            Expr::Function(func) if func.over.is_some() => {
                ...
                let resolved_over = match over {
                    ast::WindowType::NamedWindow(_) => {
                        if let Some(spec) = self.resolve_window_type(over, named_windows) {
                ...
                let new_args = match &func.args {
                    ast::FunctionArguments::List(list) => {
                        let expanded_args: Vec<ast::FunctionArg> = list
                            .args
                            .iter()
                            .map(|arg| match arg {
                                ast::FunctionArg::Unnamed(ast::FunctionArgExpr::Expr(e)) => {
                                    ast::FunctionArg::Unnamed(ast::FunctionArgExpr::Expr(
                                        self.expand_named_windows_in_expr(e, named_windows),
                                    ))
                                }
                                _ => arg.clone(),
                            })
            ...
            Expr::BinaryOp { left, op, right } => Expr::BinaryOp {
                left: Box::new(self.expand_named_windows_in_expr(left, named_windows)),
                op: op.clone(),
                right: Box::new(self.expand_named_windows_in_expr(right, named_windows)),
            },
            Expr::UnaryOp { op, expr } => Expr::UnaryOp {
                op: *op,
                expr: Box::new(self.expand_named_windows_in_expr(expr, named_windows)),
            },
            Expr::Nested(inner) => Expr::Nested(Box::new(
                self.expand_named_windows_in_expr(inner, named_windows),
            )),
            Expr::Case {
                case_token,
                end_token,
                ...
                operand: operand
                    .as_ref()
                    .map(|o| Box::new(self.expand_named_windows_in_expr(o, named_windows))),
                conditions: conditions
                    .iter()
                    .map(|cw| ast::CaseWhen {
                        condition: self.expand_named_windows_in_expr(&cw.condition, named_windows),
                        result: self.expand_named_windows_in_expr(&cw.result, named_windows),
                    })
                    .collect(),
                else_result: else_result
                    .as_ref()
                    .map(|e| Box::new(self.expand_named_windows_in_expr(e, named_windows))),
            },
            Expr::Cast {
                kind,
                expr,
    ...
    fn expand_named_windows_in_projection(
        &self,
        projection: &[SelectItem],
        named_windows: &[ast::NamedWindowDefinition],
    ) -> Vec<SelectItem> {
        ...
        projection
            ...
            .map(|item| match item {
                SelectItem::UnnamedExpr(expr) => {
                    SelectItem::UnnamedExpr(self.expand_named_windows_in_expr(expr, named_windows))
                }
                SelectItem::ExprWithAlias { expr, alias } => SelectItem::ExprWithAlias {
                    expr: self.expand_named_windows_in_expr(expr, named_windows),
                    alias: alias.clone(),
                },
                other => other.clone(),
            })
    ...
    fn create_extended_rows_with_windows(
        &self,
        input_schema: &Schema,
        rows: &[Record],
        projection: &[SelectItem],
        ...
        let mut extended_fields: Vec<Field> = input_schema.fields().to_vec();
        let mut projection_col_names: Vec<String> = Vec::new();

        for (idx, item) in projection.iter().enumerate() {
            match item {
                SelectItem::Wildcard(_) => {
                    for field in input_schema.fields() {
                        projection_col_names.push(format!("__proj_{}", field.name));
                        extended_fields.push(Field::nullable(
                            format!("__proj_{}", field.name),
                ...
                SelectItem::UnnamedExpr(expr) => {
                    let name = self.expr_to_alias(expr, idx);
                    let proj_name = format!("__proj_{}", name);
                    projection_col_names.push(proj_name.clone());
                    let data_type = self.infer_expr_type_with_windows(
                        expr,
                        input_schema,
                        rows,
                    ...
                    extended_fields.push(Field::nullable(proj_name, data_type));
                }
                SelectItem::ExprWithAlias { expr, alias } => {
                    let proj_name = format!("__proj_{}", alias.value);
                    projection_col_names.push(proj_name.clone());
                    let data_type = self.infer_expr_type_with_windows(
                        expr,
                        input_schema,
        ...
        let extended_schema = Schema::from_fields(extended_fields);
        ...
        for (row_idx, row) in rows.iter().enumerate() {
            ...
            extended_rows.push(Record::from_values(values));
        }

        Ok((extended_schema, extended_rows))
    }
    ...
    fn arrow_array_to_value(
        ...
        let value = match (array.data_type(), target_type) {
            (ArrowDataType::Boolean, _) => {
            ...
            (ArrowDataType::Time32(TimeUnit::Second), _) => {
                let arr = array.as_primitive::<arrow::datatypes::Time32SecondType>();
                let secs = arr.value(row_idx);
                let time = chrono::NaiveTime::from_num_seconds_from_midnight_opt(secs as u32, 0)
                    .unwrap_or_default();
                Value::time(time)
            }
            (ArrowDataType::Time32(TimeUnit::Millisecond), _) => {
                let arr = array.as_primitive::<arrow::datatypes::Time32MillisecondType>();
                let millis = arr.value(row_idx);
            ...
            (ArrowDataType::Time64(TimeUnit::Microsecond), _) => {
                let arr = array.as_primitive::<arrow::datatypes::Time64MicrosecondType>();
            ...
            (ArrowDataType::Time64(TimeUnit::Nanosecond), _) => {
                let arr = array.as_primitive::<arrow::datatypes::Time64NanosecondType>();
                let nanos = arr.value(row_idx);
                let secs = (nanos / 1_000_000_000) as u32;
                let nano_part = (nanos % 1_000_000_000) as u32;
                let time = chrono::NaiveTime::from_num_seconds_from_midnight_opt(secs, nano_part)
                    .unwrap_or_default();
                Value::time(time)
            }
            (ArrowDataType::Binary, _) => {
                let arr = array.as_binary::<i32>();
                Value::bytes(arr.value(row_idx).to_vec())
            }
            (ArrowDataType::LargeBinary, _) => {
                let arr = array.as_binary::<i64>();
                Value::bytes(arr.value(row_idx).to_vec())
            }
            (ArrowDataType::Decimal128(_, scale), _) => {
                let arr = array.as_primitive::<arrow::datatypes::Decimal128Type>();
                let val = arr.value(row_idx);
                let decimal = rust_decimal::Decimal::from_i128_with_scale(val, *scale as u32);
                Value::numeric(decimal)
            }
            (ArrowDataType::Decimal256(_, scale), _) => {
                let arr = array.as_primitive::<arrow::datatypes::Decimal256Type>();
                let val = arr.value(row_idx);
                let i128_val = val.as_i128();
                let decimal = rust_decimal::Decimal::from_i128_with_scale(i128_val, *scale as u32);
                Value::numeric(decimal)
            }
            (ArrowDataType::Timestamp(TimeUnit::Microsecond, _), DataType::Timestamp) => {
                ...
                let utc_dt = chrono::DateTime::from_timestamp_micros(ts).unwrap_or_default();
                Value::timestamp(utc_dt)
            }
            (ArrowDataType::Timestamp(TimeUnit::Microsecond, _), _) => {
                let arr = array.as_primitive::<arrow::datatypes::TimestampMicrosecondType>();
                let ts = arr.value(row_idx);
            ...
            (ArrowDataType::Timestamp(TimeUnit::Second, _), DataType::Timestamp) => {
                let arr = array.as_primitive::<arrow::datatypes::TimestampSecondType>();
                let ts = arr.value(row_idx);
                let utc_dt = chrono::DateTime::from_timestamp(ts, 0).unwrap_or_default();
                Value::timestamp(utc_dt)
            }
            (ArrowDataType::Timestamp(TimeUnit::Second, _), _) => {
                let arr = array.as_primitive::<arrow::datatypes::TimestampSecondType>();
                let ts = arr.value(row_idx);
            ...
            (ArrowDataType::Timestamp(TimeUnit::Millisecond, _), DataType::Timestamp) => {
                let arr = array.as_primitive::<arrow::datatypes::TimestampMillisecondType>();
                let ts = arr.value(row_idx);
                let utc_dt = chrono::DateTime::from_timestamp_millis(ts).unwrap_or_default();
                Value::timestamp(utc_dt)
            }
            (ArrowDataType::Timestamp(TimeUnit::Millisecond, _), _) => {
                let arr = array.as_primitive::<arrow::datatypes::TimestampMillisecondType>();
                let ts = arr.value(row_idx);
            ...
            (ArrowDataType::Timestamp(TimeUnit::Nanosecond, _), DataType::Timestamp) => {
                let arr = array.as_primitive::<arrow::datatypes::TimestampNanosecondType>();
                let ts = arr.value(row_idx);
                let utc_dt = chrono::DateTime::from_timestamp_nanos(ts);
                Value::timestamp(utc_dt)
            }
            (ArrowDataType::Timestamp(TimeUnit::Nanosecond, _), _) => {
                let arr = array.as_primitive::<arrow::datatypes::TimestampNanosecondType>();
                let ts = arr.value(row_idx);
    ...
    fn csv_to_value(&self, csv_val: &str, target_type: &DataType) -> Result<Value> {
        ...
        let value = match target_type {
            DataType::Unknown => Value::string(trimmed.to_string()),
            DataType::Bool => {
            ...
            DataType::Float64 => trimmed
                ...
                .map(Value::float64)
                .unwrap_or(Value::null()),
            DataType::Numeric(_) | DataType::BigNumeric => trimmed
                .parse::<rust_decimal::Decimal>()
                .map(Value::numeric)
                .unwrap_or(Value::null()),
            DataType::String => Value::string(trimmed.to_string()),
            DataType::Bytes => Value::bytes(trimmed.as_bytes().to_vec()),
            DataType::Date => {
            ...
            DataType::Time => {
                if let Ok(time) = chrono::NaiveTime::parse_from_str(trimmed, "%H:%M:%S%.f") {
                    Value::time(time)
                } else if let Ok(time) = chrono::NaiveTime::parse_from_str(trimmed, "%H:%M:%S") {
                    Value::time(time)
                } else {
                    Value::null()
                }
            ...
            DataType::Timestamp => {
                if let Ok(dt) = chrono::DateTime::parse_from_rfc3339(trimmed) {
                    Value::timestamp(dt.with_timezone(&chrono::Utc))
                } else if let Ok(dt) =
                    chrono::NaiveDateTime::parse_from_str(trimmed, "%Y-%m-%dT%H:%M:%S%.f")
                {
            ...
            DataType::Geography => Value::geography(trimmed.to_string()),
            DataType::Json => serde_json::from_str(trimmed)
                .map(Value::json)
                .unwrap_or(Value::null()),
            DataType::Struct(_) => Value::null(),
            DataType::Array(_) => Value::null(),
            DataType::Interval => Value::null(),
            DataType::Range(_) => Value::null(),
        };
    ...
    fn json_to_value(&self, json_val: &serde_json::Value, target_type: &DataType) -> Result<Value> {
        ...
        let value = match target_type {
            DataType::Unknown => match json_val {
            ...
            DataType::Numeric(_) | DataType::BigNumeric => match json_val {
            ...
            DataType::Geography => match json_val {
                serde_json::Value::String(s) => Value::geography(s.clone()),
                _ => Value::null(),
            },
            DataType::Json => Value::json(json_val.clone()),
            DataType::Struct(fields) => match json_val {
                serde_json::Value::Object(obj) => {
                    let struct_fields: Vec<(String, Value)> = fields
                        .iter()
                        .map(|field| {
                            let val = obj
                                .get(&field.name)
                                .map(|v| {
                                    self.json_to_value(v, &field.data_type).unwrap_or_default()
                                })
                                .unwrap_or(Value::null());
                            (field.name.clone(), val)
                        })
                        .collect();
                    Value::struct_val(struct_fields)
                }
                _ => Value::null(),
            },
            DataType::Array(element_type) => match json_val {
                serde_json::Value::Array(arr) => {
                    let values: Vec<Value> = arr
                        .iter()
                        .map(|v| self.json_to_value(v, element_type).unwrap_or_default())
                        .collect();
                    Value::array(values)
                }
                _ => Value::null(),
    ...
    fn field_in_except(field_name: &str, except_cols: &std::collections::HashSet<String>) -> bool {
        let name_lower = field_name.to_lowercase();
        if except_cols.contains(&name_lower) {
            return true;
        }
        if let Some(col_name) = name_lower.split('.').next_back() {
            if except_cols.contains(col_name) {
                return true;
            }
        }
        false
