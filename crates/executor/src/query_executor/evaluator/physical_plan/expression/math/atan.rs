...
impl ProjectionWithExprExec {
    pub(in crate::query_executor::evaluator::physical_plan) fn eval_atan(
        args: &[Expr],
        batch: &Table,
        row_idx: usize,
    ) -> Result<Value> {
        Self::validate_arg_count("ATAN", args, 1)?;
...
mod tests {
    ...
    fn calculates_atan_of_float() {
        ...
        let result = ProjectionWithExprExec::eval_atan(&args, &batch, 0).expect("success");
    ...
    fn validates_argument_count() {
        ...
        let err = ProjectionWithExprExec::eval_atan(&[], &batch, 0).expect_err("missing argument");
        assert_error_contains(&err, "ATAN");
    }
}
