...
impl ProjectionWithExprExec {
    pub(in crate::query_executor::evaluator::physical_plan) fn eval_asin(
        args: &[Expr],
        batch: &Table,
        row_idx: usize,
    ) -> Result<Value> {
        Self::validate_arg_count("ASIN", args, 1)?;
...
mod tests {
    ...
    #[test]
    fn calculates_asin_of_valid_float() {
        ...
        let result = ProjectionWithExprExec::eval_asin(&args, &batch, 0).expect("success");
        if let Some(f) = result.as_f64() {
            assert!((f - 0.5236).abs() < 0.001)
        } else {
    ...
    fn errors_on_out_of_range_value() {
        ...
        let args = vec![Expr::column("val")];
        let err = ProjectionWithExprExec::eval_asin(&args, &batch, 0).expect_err("out of range");
    ...
    fn validates_argument_count() {
        ...
        let err = ProjectionWithExprExec::eval_asin(&[], &batch, 0).expect_err("missing argument");
        assert_error_contains(&err, "ASIN");
    }
}
