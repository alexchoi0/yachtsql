...
impl ProjectionWithExprExec {
    pub(in crate::query_executor::evaluator::physical_plan) fn eval_safe_multiply(
        args: &[Expr],
        batch: &Table,
        row_idx: usize,
    ) -> Result<Value> {
        if args.len() != 2 {
            return Err(crate::error::Error::invalid_query(
                "SAFE_MULTIPLY requires exactly 2 arguments".to_string(),
            ));
        }
        let a = Self::evaluate_expr(&args[0], batch, row_idx)?;
        let b = Self::evaluate_expr(&args[1], batch, row_idx)?;
        crate::query_executor::execution::safe_multiply(&a, &b)
    }
}
...
mod tests {
    ...
    #[test]
    fn multiplies_ints_without_overflow() {
        let batch = create_batch(
            schema_with_two_ints(),
            vec![vec![Value::int64(10), Value::int64(20)]],
        );
        let args = vec![Expr::column("a"), Expr::column("b")];
        let result = ProjectionWithExprExec::eval_safe_multiply(&args, &batch, 0).expect("success");
        assert_eq!(result, Value::int64(200));
    }
    ...
    fn multiplies_floats() {
        ...
        let batch = create_batch(schema, vec![vec![Value::float64(2.5), Value::float64(4.0)]]);
        let args = vec![Expr::column("a"), Expr::column("b")];
        let result = ProjectionWithExprExec::eval_safe_multiply(&args, &batch, 0).expect("success");
    ...
    fn returns_null_on_int_overflow() {
        let batch = create_batch(
            schema_with_two_ints(),
            vec![vec![Value::int64(i64::MAX), Value::int64(2)]],
        );
        let args = vec![Expr::column("a"), Expr::column("b")];
        let result = ProjectionWithExprExec::eval_safe_multiply(&args, &batch, 0).expect("success");
    ...
    fn validates_argument_count() {
        ...
        let err = ProjectionWithExprExec::eval_safe_multiply(&[Expr::column("val")], &batch, 0)
            .expect_err("missing argument");
        assert_error_contains(&err, "SAFE_MULTIPLY");
    }
}
