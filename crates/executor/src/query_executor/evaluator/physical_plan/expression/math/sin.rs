...
impl ProjectionWithExprExec {
    pub(in crate::query_executor::evaluator::physical_plan) fn eval_sin(
        args: &[Expr],
        batch: &Table,
        row_idx: usize,
    ) -> Result<Value> {
        Self::validate_arg_count("SIN", args, 1)?;
...
mod tests {
    ...
    fn calculates_sin_of_zero() {
        ...
        let result = ProjectionWithExprExec::eval_sin(&args, &batch, 0).expect("success");
    ...
    #[test]
    fn calculates_sin_of_pi_over_two() {
        let schema = Schema::from_fields(vec![Field::nullable("val", DataType::Float64)]);
        let batch = create_batch(
            schema,
            vec![vec![Value::float64(std::f64::consts::FRAC_PI_2)]],
        );
        let args = vec![Expr::column("val")];
        let result = ProjectionWithExprExec::eval_sin(&args, &batch, 0).expect("success");
        if let Some(f) = result.as_f64() {
    ...
    fn validates_argument_count() {
        ...
        let err = ProjectionWithExprExec::eval_sin(&[], &batch, 0).expect_err("no args");
        assert_error_contains(&err, "SIN");
    }
}
