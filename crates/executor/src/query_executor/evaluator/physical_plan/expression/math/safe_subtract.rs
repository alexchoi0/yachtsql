...
impl ProjectionWithExprExec {
    pub(in crate::query_executor::evaluator::physical_plan) fn eval_safe_subtract(
        args: &[Expr],
        batch: &Table,
        row_idx: usize,
    ) -> Result<Value> {
        if args.len() != 2 {
            return Err(crate::error::Error::invalid_query(
                "SAFE_SUBTRACT requires exactly 2 arguments".to_string(),
            ));
        }
        let a = Self::evaluate_expr(&args[0], batch, row_idx)?;
        let b = Self::evaluate_expr(&args[1], batch, row_idx)?;
        crate::query_executor::execution::safe_subtract(&a, &b)
    }
}
...
mod tests {
    ...
    #[test]
    fn subtracts_ints_without_overflow() {
        let batch = create_batch(
            schema_with_two_ints(),
            vec![vec![Value::int64(100), Value::int64(30)]],
        );
        let args = vec![Expr::column("a"), Expr::column("b")];
        let result = ProjectionWithExprExec::eval_safe_subtract(&args, &batch, 0).expect("success");
        assert_eq!(result, Value::int64(70));
    }
    ...
    fn subtracts_floats() {
        ...
        let batch = create_batch(
            schema,
            vec![vec![Value::float64(10.5), Value::float64(3.5)]],
        );
        let args = vec![Expr::column("a"), Expr::column("b")];
        let result = ProjectionWithExprExec::eval_safe_subtract(&args, &batch, 0).expect("success");
    ...
    fn returns_null_on_int_overflow() {
        let batch = create_batch(
            schema_with_two_ints(),
            vec![vec![Value::int64(i64::MIN), Value::int64(1)]],
        );
        let args = vec![Expr::column("a"), Expr::column("b")];
        let result = ProjectionWithExprExec::eval_safe_subtract(&args, &batch, 0).expect("success");
    ...
    fn validates_argument_count() {
        ...
        let err = ProjectionWithExprExec::eval_safe_subtract(&[Expr::column("val")], &batch, 0)
            .expect_err("missing argument");
        assert_error_contains(&err, "SAFE_SUBTRACT");
    }
}
