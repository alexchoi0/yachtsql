...
impl ProjectionWithExprExec {
    pub(in crate::query_executor::evaluator::physical_plan) fn eval_exp(
        args: &[Expr],
        batch: &Table,
        row_idx: usize,
    ) -> Result<Value> {
        Self::validate_arg_count("EXP", args, 1)?;
...
mod tests {
    ...
    fn calculates_exp_of_zero() {
        ...
        let result = ProjectionWithExprExec::eval_exp(&args, &batch, 0).expect("success");
    ...
    #[test]
    fn calculates_exp_of_int() {
        let schema = Schema::from_fields(vec![Field::nullable("val", DataType::Int64)]);
        let batch = create_batch(schema, vec![vec![Value::int64(2)]]);
        let args = vec![Expr::column("val")];
        let result = ProjectionWithExprExec::eval_exp(&args, &batch, 0).expect("success");
        if let Some(f) = result.as_f64() {
            assert!((f - 7.389).abs() < 0.01)
        } else {
            panic!("Expected Float64")
        }
    }
    ...
    fn validates_argument_count() {
        ...
        let err = ProjectionWithExprExec::eval_exp(&[], &batch, 0).expect_err("missing argument");
        assert_error_contains(&err, "EXP");
    }
}
