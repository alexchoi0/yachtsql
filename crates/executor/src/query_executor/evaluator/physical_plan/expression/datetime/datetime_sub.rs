...
impl ProjectionWithExprExec {
    pub(in crate::query_executor::evaluator::physical_plan) fn eval_datetime_sub(
        args: &[Expr],
        batch: &Table,
        row_idx: usize,
    ) -> Result<Value> {
        if args.len() != 2 {
            return Err(Error::invalid_query(
                "DATETIME_SUB requires exactly 2 arguments",
            ));
        }
        ...
        let new_dt = sub_interval_from_datetime(dt, interval)?;
