...
impl ProjectionWithExprExec {
    pub(in crate::query_executor::evaluator::physical_plan) fn eval_time_add(
        args: &[Expr],
        batch: &Table,
        row_idx: usize,
    ) -> Result<Value> {
        if args.len() != 2 {
            return Err(Error::invalid_query(
                "TIME_ADD requires exactly 2 arguments",
            ));
        }
        ...
        let new_time = add_interval_to_time(time, interval)?;
