...
impl ProjectionWithExprExec {
    pub(in crate::query_executor::evaluator::physical_plan) fn eval_second(
        args: &[Expr],
        batch: &Table,
        row_idx: usize,
    ) -> Result<Value> {
        if args.len() != 1 {
            return Err(Error::invalid_query(
                "SECOND requires exactly 1 argument (timestamp)".to_string(),
            ));
        }
        ...
        if let Some(ts) = ts_val.as_timestamp() {
            Ok(Value::int64(ts.second() as i64))
        } else if let Some(t) = ts_val.as_time() {
            Ok(Value::int64(t.second() as i64))
        } else {
