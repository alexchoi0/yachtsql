...
impl ProjectionWithExprExec {
    pub(in crate::query_executor::evaluator::physical_plan) fn eval_date_sub(
        args: &[Expr],
        batch: &Table,
        row_idx: usize,
    ) -> Result<Value> {
        if args.len() != 2 {
            return Err(Error::invalid_query(
                "DATE_SUB requires exactly 2 arguments (date, interval)".to_string(),
            ));
        }
        ...
        match date_val.as_date() {
            Some(d) => {
                let new_date =
                    crate::query_executor::execution::sub_interval_from_date(d, &interval)?;
...
mod tests {
    ...
    #[test]
    fn subtracts_days_from_date() {
        let base_date = chrono::NaiveDate::from_ymd_opt(2024, 1, 25).unwrap();
        ...
        let result = ProjectionWithExprExec::eval_date_sub(&args, &batch, 0).expect("success");
    ...
    fn validates_argument_count() {
        ...
        let err = ProjectionWithExprExec::eval_date_sub(&[Expr::column("date_col")], &batch, 0)
            .expect_err("missing argument");
        assert_error_contains(&err, "DATE_SUB");
    ...
    fn errors_on_non_date_first_argument() {
        ...
        let err =
            ProjectionWithExprExec::eval_date_sub(&args, &batch, 0).expect_err("type mismatch");
