...
impl ProjectionWithExprExec {
    pub(in crate::query_executor::evaluator::physical_plan) fn evaluate_json_query(
        args: &[Expr],
        batch: &Table,
        row_idx: usize,
    ) -> Result<Value> {
        if args.len() != 2 {
            return Err(Error::invalid_query(
                "JSON_QUERY requires exactly 2 arguments".to_string(),
            ));
        }
        ...
        yachtsql_functions::json::json_query(&json_val, path_str)
    }
}
...
mod tests {
    ...
    fn extracts_json_object() {
        ...
        let result =
            ProjectionWithExprExec::evaluate_json_query(&args, &batch, 0).expect("success");
    ...
    fn extracts_json_array() {
        let batch = create_batch(
            schema_with_json_and_path(),
            vec![vec![
                Value::string(r#"{"items":[1,2,3,4,5]}"#.into()),
    ...
    fn validates_argument_count() {
        ...
        let err = ProjectionWithExprExec::evaluate_json_query(&[Expr::column("json")], &batch, 0)
            .expect_err("missing argument");
        assert_error_contains(&err, "JSON_QUERY requires exactly 2 arguments");
    }
}
