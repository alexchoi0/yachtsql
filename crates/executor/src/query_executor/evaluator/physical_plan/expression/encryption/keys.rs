...
impl ProjectionWithExprExec {
    pub(in crate::query_executor::evaluator::physical_plan) fn evaluate_keys_function(
        ...
        Err(Error::unsupported_feature(
            "KEYS.* functions need migration from core.rs".to_string(),
        ))
    }
}
...
mod tests {
    ...
    fn returns_unsupported_feature_error() {
        ...
        let err = ProjectionWithExprExec::evaluate_keys_function("KEYSET_CHAIN", &args, &batch, 0)
            .expect_err("not yet implemented");
        assert_error_contains(&err, "KEYS");
