...
mod tests {
    ...
    fn test_sha256_null() {
        ...
        let result = ProjectionWithExprExec::eval_sha256(&args, &batch, 2);
        assert!(result.is_ok());
        assert_eq!(result.unwrap(), Value::null());
    }

    #[test]
    fn test_sha256_no_args() {
        let batch = create_test_batch();
        let args = vec![];
        let result = ProjectionWithExprExec::eval_sha256(&args, &batch, 0);
        ...
        match result {
            Err(Error::InvalidQuery(msg)) => {
                assert!(msg.contains("SHA256 requires exactly 1 argument"))
            }
