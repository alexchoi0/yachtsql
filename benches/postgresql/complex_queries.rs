...
pub fn bench_subquery_scalar(c: &mut Criterion) {
    let mut group = c.benchmark_group("pg_complex_subquery_scalar");
...
pub fn bench_subquery_in(c: &mut Criterion) {
    let mut group = c.benchmark_group("pg_complex_subquery_in");
    for &rows in &ROW_COUNTS {
        let mut executor = create_executor();
        setup_join_tables(&mut executor, rows);
        group.bench_with_input(BenchmarkId::from_parameter(rows), &rows, |b, _| {
...
pub fn bench_cte_simple(c: &mut Criterion) {
    let mut group = c.benchmark_group("pg_complex_cte_simple");
    for &rows in &ROW_COUNTS {
        let mut executor = create_executor();
        setup_join_tables(&mut executor, rows);
        group.bench_with_input(BenchmarkId::from_parameter(rows), &rows, |b, _| {
...
pub fn bench_cte_with_join(c: &mut Criterion) {
    let mut group = c.benchmark_group("pg_complex_cte_join");
    for &rows in &ROW_COUNTS {
        let mut executor = create_executor();
        setup_join_tables(&mut executor, rows);
        group.bench_with_input(BenchmarkId::from_parameter(rows), &rows, |b, _| {
...
pub fn bench_union(c: &mut Criterion) {
    let mut group = c.benchmark_group("pg_complex_union");
    for &rows in &ROW_COUNTS {
        let mut executor = create_executor();
        setup_join_tables(&mut executor, rows);
        group.bench_with_input(BenchmarkId::from_parameter(rows), &rows, |b, _| {
...
pub fn bench_case_expression(c: &mut Criterion) {
    let mut group = c.benchmark_group("pg_complex_case");
    for &rows in &ROW_COUNTS {
        let mut executor = create_executor();
        setup_join_tables(&mut executor, rows);
        group.bench_with_input(BenchmarkId::from_parameter(rows), &rows, |b, _| {
