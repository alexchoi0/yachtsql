...
pub fn bench_string_upper(c: &mut Criterion) {
    let mut group = c.benchmark_group("ch_func_upper");
...
pub fn bench_string_lower(c: &mut Criterion) {
    let mut group = c.benchmark_group("ch_func_lower");
    for &rows in &ROW_COUNTS {
        let mut executor = create_executor();
        setup_users_table(&mut executor, rows);
        group.bench_with_input(BenchmarkId::from_parameter(rows), &rows, |b, _| {
...
pub fn bench_string_length(c: &mut Criterion) {
    let mut group = c.benchmark_group("ch_func_length");
    for &rows in &ROW_COUNTS {
        let mut executor = create_executor();
        setup_users_table(&mut executor, rows);
        group.bench_with_input(BenchmarkId::from_parameter(rows), &rows, |b, _| {
...
pub fn bench_string_concat(c: &mut Criterion) {
    let mut group = c.benchmark_group("ch_func_concat");
    for &rows in &ROW_COUNTS {
        let mut executor = create_executor();
        setup_users_table(&mut executor, rows);
        group.bench_with_input(BenchmarkId::from_parameter(rows), &rows, |b, _| {
...
pub fn bench_string_substring(c: &mut Criterion) {
    let mut group = c.benchmark_group("ch_func_substring");
    for &rows in &ROW_COUNTS {
        let mut executor = create_executor();
        setup_users_table(&mut executor, rows);
        group.bench_with_input(BenchmarkId::from_parameter(rows), &rows, |b, _| {
...
pub fn bench_math_abs(c: &mut Criterion) {
    let mut group = c.benchmark_group("ch_func_abs");
    for &rows in &ROW_COUNTS {
        let mut executor = create_executor();
        setup_users_table(&mut executor, rows);
        group.bench_with_input(BenchmarkId::from_parameter(rows), &rows, |b, _| {
...
pub fn bench_coalesce(c: &mut Criterion) {
    let mut group = c.benchmark_group("ch_func_coalesce");
    for &rows in &ROW_COUNTS {
        let mut executor = create_executor();
        setup_users_table(&mut executor, rows);
        group.bench_with_input(BenchmarkId::from_parameter(rows), &rows, |b, _| {
