...
#[cfg(feature = "visitor")]
use yachtsql_sqlparser_derive::{Visit, VisitMut};
...
pub enum Statement {
    ...
    /// An `ITERATE` statement (continues to next loop iteration).
    Iterate {
        label: Option<Ident>,
    },
    /// A `BREAK` statement (exits a loop, alias for LEAVE in BigQuery).
    Break {
        label: Option<Ident>,
    },
    /// A `CONTINUE` statement (continues to next loop iteration).
    Continue {
        label: Option<Ident>,
    },
    /// A `RAISE` statement.
